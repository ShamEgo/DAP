...
# Define tower assignments
tower_assignments = {
    'tab-1': ['TAS004699_STR_1', 'TAS004686_STR_1'],
    'tab-3': ['NT002793_STR_1', 'NT006415_STR_1', 'NT003447_STR_1'],
    'tab-4': ['QLD002720_STR_1', 'QLD002533_STR_1'],
    'tab-5': ['NSW002537_STR_1', 'NSW002494_STR_1'],
    'tab-6': ['QLD007207_STR_1', 'QLD002376_STR_1'],
    'tab-7': ['QLD002902_STR_1', 'QLD002591_STR_1'],
    'tab-8': ['QLD002837_STR_1', 'QLD005674_STR_1'],
    'tab-9': ['QLD003537_STR_1', 'QLD006031_STR_1'],
    'tab-10': ['VIC002609_STR_1', 'VIC003102_STR_1'],
}
...
# Cache HTML file existence at startup
html_files = {
    asset: os.path.exists(os.path.join('assets', f'scattermapbox_plot_{asset}.html'))
    for asset in set(sum(tower_assignments.values(), []))
}
...
def create_weather_plot(tower, silo_df, corrosion_df, start_date):
    ...
    for issue_date in tower_corrosion_df['IssueCreated']:
...
@app.callback(
    Output('tower-tiles', 'children'),
    [Input('tabs', 'value')]
)
def update_tower_tiles(tab):
    ...
    tower_info = df.groupby('AMSAssetRef').agg({
        'CorrosionRegionType': 'first',
        'PaintingType': 'first',
        'StructureClassCode': 'first',
        'Height': 'first'
    }).reset_index()

    tiles = []
    for _, row in tower_info.iterrows():
        tile_content = [
            html.H4(row['AMSAssetRef'], style={'margin': '0', 'fontSize': '1rem', 'fontWeight': 'bold', 'font-family': 'Roboto, sans-serif'}),
            html.P(f"Corrosion: {row['CorrosionRegionType']}", style={'margin': '2px 0', 'fontSize': '0.8rem', 'font-family': 'Roboto, sans-serif'}),
            html.P(f"Painting: {row['PaintingType']}", style={'margin': '2px 0', 'fontSize': '0.8rem', 'font-family': 'Roboto, sans-serif'}),
            html.P(f"Class: {row['StructureClassCode']}", style={'margin': '2px 0', 'fontSize': '0.8rem', 'font-family': 'Roboto, sans-serif'}),
            html.P(f"Height: {row['Height']}", style={'margin': '2px 0', 'fontSize': '0.8rem', 'font-family': 'Roboto, sans-serif'})
        ]
        tile_row = html.Div(
        ...
        tiles.append(
            html.Div(
                ...
                style={
                    ...
                    'border': '1px solid #FFFFFF',
                    'flex': '1',
                    'minWidth': f'calc(99vw / {len(towers_selected)} - 14px)',
                    'font-family': 'Roboto, sans-serif'
                }
            )
        )
    return html.Div(
        tiles,
        style={'display': 'flex', 'flexDirection': 'row', 'gap': '5px', 'width': '100%', 'font-family': 'Roboto, sans-serif'}
    )
...
def render_map_content(tab):
    ...
    # Create card tiles for distance_km (proximity to coast)
    distance_tiles = [
        html.Div(
            f"{row['AMSAssetRef']}: {row['distance_km']:.2f} km to coast" if pd.notna(row['distance_km']) else f"{row['AMSAssetRef']}: Unknown distance",
            style={
                'backgroundColor': '#000000',
                'color': '#FFFFFF',
                'fontSize': '0.9rem',
    ...
    # Create dark map with red lines and distance annotations
    dark_map = go.Figure()
    dark_map.add_trace(
        go.Scattermapbox(
            lat=unique_df['Latitude'],
            lon=unique_df['Longitude'],
            text=unique_df['AMSAssetRef'],
            mode='markers+text',
    ...
    return html.Div([
        ...
        dcc.Graph(
            figure=dark_map,
            style={'width': '100%', 'height': 'calc(98vh - 120px)'}
        )
    ])
...
def render_satellite_maps(tab):
    ...
    for asset in towers_selected:
        ...
        if asset_df.empty:
            satellite_maps.append(
                html.Div([
                    html.H4(asset, style={'margin': '0', 'fontSize': '1rem', 'fontWeight': 'bold', 'textAlign': 'center', 'font-family': 'Roboto, sans-serif'}),
        ...
        fig.update_layout(
            title=dict(text='', x=0.5, xanchor='center', font=dict(size=10, family='Roboto, sans-serif')),
            mapbox_style='satellite',
            mapbox_accesstoken='pk.eyJ1IjoiYW51YmhhdmpldGxleSIsImEiOiJjbWFraHpzbmkwOHRlMmtvaDhhaDY1ajM0In0.MolCp0Po3LjGf8z5ebKJig',
            mapbox=dict(
                style='satellite',
                zoom=15,
                center=dict(lat=asset_df['Latitude'].iloc[0], lon=asset_df['Longitude'].iloc[0]),
                pitch=55,
            ),
            plot_bgcolor='#1a1a1a',
            paper_bgcolor='#1a1a1a',
        ...
        satellite_maps.append(
            html.Div([
                ...
                dcc.Graph(
                    figure=fig,
                    style={'width': '100%', 'height': f'calc((98vh - 120px - 30px) / {len(towers_selected)})', 'margin': '2px'}
                )
            ])
        )
    ...
    return html.Div(
        satellite_maps,
        style={'display': 'flex', 'flexDirection': 'column', 'gap': '5px', 'width': '32vw', 'height': '100%', 'font-family': 'Roboto, sans-serif'}
    )
...
def render_weather_content(tab):
    ...
    for tower in towers_selected:
        ...
        rainfall_fig.update_layout(title=dict(text=f"{tower} Rainfall", x=0.5, xanchor='center', font=dict(size=10, family='Roboto, sans-serif')))
        weather_plots.append(
            dcc.Graph(
                figure=rainfall_fig,
                style={'width': '33.33vw', 'height': 'calc(50% - 2px)', 'margin': '2px'}
            )
        )

    return html.Div(
        weather_plots,
        style={'display': 'flex', 'flexDirection': 'column', 'gap': '5px', 'width': '32vw', 'height': '100%', 'overflow': 'hidden', 'font-family': 'Roboto, sans-serif'}
    )
